...
- Máximo 4-5 líneas por respuesta. Si hay pasos, los numerás en línea (1) 2) 3))
...
- Si no sabes algo sobre el caso clínico del paciente, lo dices y derivas al médico
...
━━━━ MÓDULO: WHITATSAPP (/dashboard/medic/whatsapp) ━━━━
  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { reply: 'Ash no está disponible en este momento. Contacta a soporte si el problema persiste.' },
        { status: 200 }
      );
    }

    // Inject dynamic context into system prompt
    const systemPrompt = ASH_DASHBOARD_SYSTEM_PROMPT
    ...
    if (!response.ok) {
      ...
      console.error('[ash-dashboard] Gemini error:', response.status, errText);
      return NextResponse.json(
        { reply: 'Tuve un problema procesando tu pregunta. Intenta de nuevo en un momento.' },
        { status: 200 }
      );
    }
    ...
    console.error('[ash-dashboard] Error:', err);
    return NextResponse.json(
