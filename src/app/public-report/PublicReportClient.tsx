...
export default function PublicReportClient({ clinicaId, expiresAt, sig }: PublicReportClientProps) {
  ...
  const handleCapture = async () => {
    setCapturing(true);
    try {
      const m = data?.metricas || { totalCitas: 0, confirmadas: 0, noAsistio: 0, revenue: 0, pacientesNuevos: 0 };
      
      const reportData = {
        organizationName: `Clínica ${clinicaId.slice(0, 8)}`,
        periodCurrent: weekLabel,
        periodPrevious: "Periodo Anterior",
        metrics: {
          current: {
            totalCitas: m.totalCitas || 0,
            confirmadas: m.confirmadas || 0,
            noAsistio: m.noAsistio || 0,
            revenue: m.revenue || 0,
            pacientesNuevos: m.pacientesNuevos || 0,
        ...
        agenda: [],
        trends: {
          monthlyVolume: (trendsData?.monthlyTrend || []).map((v: any) => ({ month: v.month, value: v.value })),
          topServices: (trendsData?.topServices || []).map((s: any) => ({ name: s.name, value: s.value })),
          dayCounts: []
        },
        loyalty: {
          recurrentes: ltvData?.loyalty?.recurrentes || 0,
          unicos: ltvData?.loyalty?.unicos || 0
        }
      };
      ...
      link.href = url;
      link.download = `Reporte_Gerencial_${clinicaId.slice(0, 8)}_${weekLabel.replace(/\s+/g, '_')}.pdf`;
      link.click();
