...
export default function NewSpecialistPage() {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      const response = await fetch('/api/administration/specialists', {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Error al crear el especialista');
      }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-2 text-lg">
            Registra un nuevo profesional clínico en tu organización.
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-[11px] text-slate-500 mt-1">Este correo se usará para notificaciones (no otorga acceso al sistema por sí solo).</p>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <input 
                  {...register('inpresSax')}
                  className={cn(
                    "w-full rounded-xl border px-4 py-3 bg-slate-50 outline-none transition-all focus:ring-4 focus:ring-teal-100 focus:bg-white text-sm uppercase",
                    errors.inpresSax ? "border-red-500" : "border-slate-200"
                  )}
                  placeholder="Código de registro médico"
                />
                {errors.inpresSax && <p className="text-xs font-bold text-red-500 mt-1">{errors.inpresSax.message}</p>}
              </div>
          ...
          <div className="pt-8 border-t border-slate-100 flex justify-end">
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <Save className="w-6 h-6" /> CREAR ESPECIALISTA
                </>
              )}
            </button>
          </div>
