...
export default function PointsWidget() {
  ...
  return (
    <div className="relative rounded-2xl shadow-xl overflow-hidden text-white group">
      ...
      {/* Contenido Principal */}
      <div className="relative z-10 p-3 sm:p-4 md:p-5 flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          ...
          <div className="relative w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center flex-shrink-0">
            ...
            <HeartPulse className="w-6 h-6 sm:w-7 sm:h-7 text-[#7FFFD4] relative z-10 drop-shadow-lg" strokeWidth={2.5} />
          </div>
          
          <div className="flex flex-col justify-center">
            <div className="flex items-center gap-2 mb-0.5">
              <span className="w-1 h-1 rounded-full bg-[#7FFFD4] animate-pulse"></span>
              <p className="text-white/80 text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em]">Tus Pulsos</p>
            </div>
            <p className="text-3xl sm:text-4xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-white to-[#7FFFD4] drop-shadow-sm leading-none">
        ...
        <div className="flex-1 w-full max-w-lg hidden md:block px-6 border-l border-white/10">
          <div className="flex justify-between items-end mb-1.5">
            <div className="flex flex-col">
              <span className="text-white/60 text-[9px] font-bold uppercase tracking-widest mb-0.5">Nivel {currentLevel.level}</span>
              <p className="text-white font-bold text-base leading-none">{currentLevel.name}</p>
            </div>
            {nextLevel ? (
              <div className="text-right">
                <span className="text-white/60 text-[9px] font-bold uppercase tracking-widest mb-0.5">Siguiente Nivel</span>
                <p className="text-[#7FFFD4] text-xs font-semibold">
                  {summary.total_earned} <span className="text-white/50 font-normal">/ {nextLevel.minPoints}</span>
                </p>
              </div>
            ) : (
              <div className="text-right">
                <span className="text-[#7FFFD4] text-[10px] font-bold uppercase tracking-wider bg-[#7FFFD4]/10 px-2 py-1 rounded-md">Nivel Máximo</span>
              </div>
            )}
          </div>
          
          <div className="relative h-2 w-full bg-black/20 rounded-full overflow-hidden backdrop-blur-sm border border-white/5">
            <div 
        ...
        <div className="flex flex-col w-full md:w-auto gap-3 md:gap-0 justify-between h-full">
          {/* Progress for Mobile only */}
          <div className="md:hidden flex-1 w-full bg-white/5 rounded-xl p-2.5 border border-white/10">
             <div className="flex justify-between items-center mb-1.5">
               <div>
                 <span className="text-white/60 text-[8px] uppercase tracking-wider block">Nivel {currentLevel.level}</span>
                 <span className="text-xs font-bold">{currentLevel.name}</span>
               </div>
               <span className="text-[#7FFFD4] text-[10px] font-semibold">{Math.round(progress)}%</span>
             </div>
          ...
          <Link 
            ...
            className="group/btn relative overflow-hidden flex items-center justify-center gap-2 bg-white text-blue-900 px-4 py-2 sm:py-2.5 rounded-xl font-bold transition-all shadow-[0_4px_15px_rgba(0,0,0,0.1)] hover:shadow-[0_6px_20px_rgba(127,255,212,0.3)] hover:-translate-y-0.5 w-full md:w-auto"
          >
            ...
            <span className="relative z-10 text-[11px] sm:text-xs uppercase tracking-wider">Salud+</span>
            <ChevronRight className="w-3.5 h-3.5 relative z-10 group-hover/btn:translate-x-1 transition-transform" />
          </Link>
        </div>
      </div>
    </div>
