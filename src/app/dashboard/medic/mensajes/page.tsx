			...
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<div>
					<div className="flex items-center gap-3 mb-2">
						<div className="p-2 bg-gradient-to-r from-teal-600 to-cyan-600 rounded-xl text-white">
					...
					<p className="text-slate-600">Gestiona tus conversaciones con pacientes y colegas</p>
				</div>
			</div>
			...
			<div className="bg-white rounded-2xl border border-blue-100 p-4 shadow-sm">
				<div className="relative">
					...
					<input
						...
						className="w-full pl-10 pr-4 py-3 border border-blue-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 text-slate-900 bg-white"
					/>
				</div>
			</div>
						...
						{filteredConversations.map((conv) => (
							<Link key={conv.id} href={`/dashboard/medic/mensajes/${conv.id}`}>
								<motion.div
									...
									exit={{ opacity: 0, y: -10 }}
									className="bg-white rounded-2xl border border-blue-100 p-6 hover:shadow-lg transition-all cursor-pointer group"
								>
									<div className="flex items-start justify-between gap-4">
										<div className="flex items-start gap-4 flex-1 min-w-0">
											...
											<div className="flex-shrink-0">
												{conv.patient ? (
													<div className="w-14 h-14 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center text-white font-semibold shadow-md">
													...
													<div className="w-14 h-14 rounded-full bg-gradient-to-br from-teal-500 to-cyan-500 flex items-center justify-center text-white shadow-md">
														<Stethoscope className="w-7 h-7" />
													</div>
												) : (
													<div className="w-14 h-14 rounded-full bg-gradient-to-br from-slate-400 to-slate-500 flex items-center justify-center text-white shadow-md">
											...
											<div className="flex-1 min-w-0">
												<div className="flex items-center gap-2 mb-2">
													...
													{conv.unreadCount > 0 && (
														<span className="px-2.5 py-1 bg-red-500 text-white rounded-full text-xs font-bold flex-shrink-0">
															{conv.unreadCount}
														</span>
													)}
												</div>
												{conv.lastMessage && (
													<>
														<p className="text-sm text-slate-600 mb-3 line-clamp-2 group-hover:text-slate-900 transition-colors">
