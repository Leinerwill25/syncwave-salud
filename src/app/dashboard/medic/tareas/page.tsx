		...
		<div className="space-y-4 sm:space-y-6">
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
				<div>
					...
					<p className="text-sm sm:text-base text-slate-600 mt-1">Gestiona tus tareas y recordatorios</p>
				</div>
			...
			{loading ? (
				...
				<div className="bg-white rounded-xl sm:rounded-2xl border border-blue-100 p-8 sm:p-12 text-center">
					<CheckSquare className="w-12 h-12 sm:w-16 sm:h-16 text-slate-300 mx-auto mb-4" />
					<p className="text-slate-600 text-base sm:text-lg">No hay tareas</p>
							...
							>
								<div className="flex items-start gap-3 sm:gap-4">
									<button
										...
										className={`mt-1 w-4 h-4 sm:w-5 sm:h-5 rounded border-2 flex items-center justify-center transition shrink-0 ${
											task.completed
												? 'bg-teal-600 border-teal-600'
												: 'border-blue-300 hover:border-teal-600'
										}`}
									>
										{task.completed && <CheckSquare className="w-3 h-3 sm:w-4 sm:h-4 text-white" />}
									</button>
									<div className="flex-1 min-w-0">
										...
										{task.description && (
											<p className="text-xs sm:text-sm text-slate-600 mb-2">{task.description}</p>
										)}
										{task.Patient && (
										...
										<div className="flex flex-wrap items-center gap-3 sm:gap-4 mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-blue-100">
											{task.due_at && (
