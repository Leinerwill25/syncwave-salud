	...
	return (
		<div className="space-y-4">
			{appointments.map((appt) => (
				<motion.div key={appt.id} whileHover={{ y: -3 }} className="rounded-2xl bg-white/90 backdrop-blur-sm border border-gray-100 shadow-sm hover:shadow-lg transition-all p-4 flex justify-between items-center">
					<div>
						<h3 className="text-base font-semibold text-gray-800">{appt.patient}</h3>
						...
						<div className="flex gap-3 mt-2 text-gray-400 text-xs">
							...
							{appt.location && (
								<span className="flex items-center gap-1">
									<MapPin className="w-4 h-4" /> {appt.location}
								</span>
							)}
						</div>
					</div>
					...
					<div className="relative">
						{loadingId === appt.id ? (
							<div className="flex items-center justify-center w-28 h-8 rounded-xl bg-gray-100 text-gray-400">
								<Loader2 className="w-4 h-4 animate-spin" />
							</div>
						) : (
							<select value={appt.status} onChange={(e) => handleStatusChange(appt.id, e.target.value)} className={`appearance-none text-white text-xs font-semibold px-3 py-1 rounded-xl cursor-pointer bg-linear-to-r ${getStatusColor(appt.status)} focus:ring-2 focus:ring-offset-1 focus:ring-indigo-500`} style={{ paddingRight: '1.5rem' }}>
								{statusOptions.map((s) => (
