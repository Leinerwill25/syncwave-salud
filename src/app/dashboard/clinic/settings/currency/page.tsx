/**
 * Página de configuración de moneda para Clínicas
 * Permite a la clínica seleccionar la moneda principal del sistema desde las monedas disponibles.
 */
...
export default function ClinicCurrencyConfigPage() {
	...
	const loadPreference = async () => {
		try {
			...
			// Obtener preferencia de la clínica desde su perfil
			const res = await fetch('/api/clinic-profile');
			if (!res.ok) throw new Error('Error fetching clinic profile');
			const data = await res.json();

			if (data.ok && data.profile?.currency) {
				setPreference(data.profile.currency);
			}
		} catch (err) {
			console.error('Error cargando preferencia de moneda de la clínica:', err);
			setMessage({ type: 'error', text: 'Error al cargar la configuración de la clínica' });
		} finally {
			setLoading(false);
		}
	};
	...
	const savePreference = async (newPreference: string) => {
		try {
			...
			// Actualizar preferencia en la base de datos (clinic_profile)
			const res = await fetch('/api/clinic-profile', {
				method: 'PUT',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ currency: newPreference }),
			});

			if (!res.ok) {
				throw new Error('Error updating clinic profile');
			}

			setPreference(newPreference);
			setMessage({ type: 'success', text: 'Preferencia de moneda guardada correctamente para la clínica' });

			// Guardar también en localStorage para acceso rápido
			localStorage.setItem('clinic_currency_preference', newPreference);
			...
			<div className="max-w-5xl mx-auto space-y-6">
				...
				>
					<div className="flex items-center gap-4 mb-2">
						...
						<div className="flex-1">
							...
							<p className="text-sm sm:text-base text-slate-600 mt-1">Selecciona la moneda principal de cotización para la clínica</p>
						</div>
					</div>
				</motion.div>
						...
						<div className="space-y-6">
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								...
								<div className="p-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-200">
									...
									<p className="text-xs text-slate-500">
										Basado en la moneda preferida: <strong>{preference}</strong>
									</p>
								</div>
							</div>
						</div>
				...
				>
					<div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-200">
						...
						<h2 className="text-xl font-bold text-slate-900">Moneda de Preferencia Institucional</h2>
					</div>

					<p className="text-sm text-slate-600 mb-6">
						Selecciona la moneda en la que deseas que se muestren y cotizen los precios de los servicios de la clínica. Esta preferencia se aplicará a todos los pacientes y especialistas asociados.
					</p>
						...
						<div className="space-y-4">
							<div className="relative">
								<label className="block text-sm font-semibold text-slate-700 mb-2">
									Selecciona la moneda preferida
								</label>
								<div className="relative">
									<select
										value={preference}
				...
				>
					...
					<ul className="space-y-3 text-sm text-slate-700">
						<li className="flex items-start gap-3">
							...
							<span>
								<strong className="font-semibold">Actualización Automática:</strong> Las tasas de cambio se sincronizan automáticamente con las referencias oficiales.
							</span>
						</li>
						<li className="flex items-start gap-3">
							<div className="w-1.5 h-1.5 bg-blue-600 rounded-full mt-2 shrink-0" />
							<span>
								<strong className="font-semibold">Facturación Consolidada:</strong> La moneda seleccionada será la predeterminada para todos los servicios listados en tu perfil de "Clientes Propios".
							</span>
						</li>
					</ul>
				</motion.div>
