...
export async function POST(request: Request) {
	try {
		...
		try {
			...
			console.debug('[Enable 2FA API] Error leyendo sb-access-token:', err);
		}
		...
		if (authError) {
			console.error('[Enable 2FA API] Auth error:', authError);
		}
		...
		// Por ahora, solo validamos que el código tenga 6 dígitos
		// En producción, deberías:
		// 1. Generar un secreto TOTP para el usuario
		// 2. Guardarlo en la base de datos
		// 3. Validar el código usando una librería como 'otplib'
		// 4. Guardar el estado de 2FA habilitado en la tabla User
		...
		// await supabase.from('users').update({ twoFactorEnabled: true }).eq('id', appUser.id);
		...
		console.error('[Enable 2FA API] Error:', err);
