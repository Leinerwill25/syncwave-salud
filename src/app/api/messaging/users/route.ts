...
export async function GET(req: NextRequest) {
	try {
		...
		if (currentUserType === 'role_user') {
			...
			const { data: doctors } = await supabase.from('users').select('id, name, role').eq('"organizationId"', organizationId).eq('role', 'MEDICO');
			...
			// Otros doctores
			const { data: otherDoctors } = await supabase.from('users').select('id, name, role').eq('"organizationId"', organizationId).eq('role', 'MEDICO').neq('id', currentUserId);
