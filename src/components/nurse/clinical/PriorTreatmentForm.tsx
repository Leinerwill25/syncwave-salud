...
export function PriorTreatmentForm({ initialData, onSubmit, isLoading }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onLocalSubmit)} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2 md:col-span-2">
          ...
          <input 
            ...
            className={cn(
              "w-full rounded-xl border px-4 py-3 bg-white  outline-none transition-all focus:ring-4 focus:ring-purple-100  text-lg font-semibold",
              errors.medication_name ? "border-red-500" : "border-gray-200  focus:border-purple-500"
            )}
          />
        ...
        <div className="space-y-2">
          <label className="text-sm font-bold text-gray-900 ">Dosis</label>
          <input 
            {...register('dose')}
            placeholder="Ej: 1 tableta"
            className="w-full rounded-xl border border-gray-200  px-4 py-3 bg-white  outline-none focus:border-purple-500 focus:ring-4 focus:ring-purple-100  transition-all font-medium"
          />
        </div>

        <div className="space-y-2">
          <label className="text-sm font-bold text-gray-900 ">Frecuencia</label>
          <input 
            {...register('frequency')}
        ...
        {/* Route & Status */}
        <div className="space-y-2">
          <label className="text-sm font-bold text-gray-900 ">Vía de Adm.</label>
          <select 
            {...register('route')}
            className="w-full rounded-xl border border-gray-200  px-4 py-3 bg-white  outline-none focus:border-purple-500 focus:ring-4 focus:ring-purple-100  transition-all"
          >
        ...
        <div className="space-y-2">
          <label className="text-sm font-bold text-gray-900 ">Estado del Tratamiento</label>
          <select 
      ...
      {/* Interaction Check */}
      <label className="flex items-center gap-3 p-4 bg-amber-50  border border-amber-200  rounded-2xl cursor-pointer group hover:bg-amber-100 transition-colors">
        <input 
          type="checkbox"
        ...
        <div>
          ...
          <p className="text-[10px] text-amber-700 ">Marque si el paciente toma otros fármacos que podrían colisionar con este.</p>
        </div>
      </label>

      {/* Notes */}
      <div className="space-y-2">
        <label className="text-sm font-bold text-gray-900 ">Observaciones</label>
        <textarea 
          {...register('notes')}
          rows={2}
          placeholder="Notas adicionales sobre la tolerancia al medicamento..."
          className="w-full rounded-xl border border-gray-200  bg-white  p-4 text-sm outline-none focus:ring-4 focus:ring-purple-100  transition-all resize-none"
        />
      </div>
