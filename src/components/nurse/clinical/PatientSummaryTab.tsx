...
export function PatientSummaryTab({ patient }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-12">
      ...
      <section className="bg-white  rounded-3xl border border-gray-100  p-6 md:p-8 space-y-4">
        <div className="flex items-center gap-3 mb-2">
          <div className="p-2 bg-blue-50  rounded-xl">
            <History className="w-5 h-5 text-blue-600" />
          </div>
          <h2 className="text-lg font-bold text-gray-900 ">Motivo de Consulta Actual</h2>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 pt-4">
          ...
          <div className="p-3 bg-white  rounded-xl border border-gray-100 ">
            ...
            <p className="text-sm font-bold text-gray-900 ">{patient.blood_type || 'N/A'}</p>
          </div>
        </div>
      </section>

      {/* Origin Section */}
      <section className="bg-white  rounded-3xl border border-gray-100  p-6 md:p-8 space-y-4 relative overflow-hidden">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-teal-50  rounded-xl">
              <MapPin className="w-5 h-5 text-teal-600" />
            </div>
            <h2 className="text-lg font-bold text-gray-900 ">Origen del Paciente</h2>
          </div>
          
          <Dialog open={showOriginModal} onOpenChange={setShowOriginModal}>
            <DialogTrigger asChild>
              <button className="p-2 hover:bg-gray-100  rounded-full transition-colors text-teal-600">
               ...
               {originData.origin.origin_org_name && (
                 <div className="flex gap-3">
                   ...
                   <div>
                     ...
                     <p className="text-sm font-semibold text-gray-700 ">{originData.origin.origin_org_name}</p>
                   </div>
                 </div>
               )}
               {originData.origin.referring_doctor_name && (
                 <div className="flex gap-3">
                   ...
                   <div>
                     ...
                     <p className="text-sm font-semibold text-gray-700 ">{originData.origin.referring_doctor_name}</p>
                   </div>
                 </div>
               )}
            </div>
      ...
      <section className="lg:col-span-2 bg-white  rounded-3xl border border-gray-100  p-6 md:p-8 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-purple-50  rounded-xl">
              <Pill className="w-5 h-5 text-purple-600" />
            </div>
            <h2 className="text-lg font-bold text-gray-900 ">Tratamientos Previos y Activos</h2>
          </div>
          
          <div className="flex items-center gap-3">
            {originData && originData.active_treatments_count > 0 && (
              <span className="hidden sm:inline-block px-3 py-1 bg-emerald-100  text-emerald-700  text-xs font-bold rounded-full">
            ...
            <Dialog open={showTreatmentModal} onOpenChange={setShowTreatmentModal}>
              <DialogTrigger asChild>
                <button className="flex items-center gap-2 px-4 py-2 bg-purple-50 hover:bg-purple-100   text-purple-700  rounded-xl text-sm font-bold transition-all">
        ...
        {originData?.treatments && originData.treatments.length > 0 ? (
            ...
            {originData.treatments.map((treatment) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h4 className="font-bold text-gray-900 ">{treatment.medication_name}</h4>
          ...
          <div className="py-12 flex flex-col items-center justify-center bg-gray-50  border-2 border-dashed border-gray-200  rounded-3xl">
            <Pill className="w-12 h-12 text-gray-200  mb-4" />
            <p className="text-gray-500  font-medium">No hay historial de tratamientos previos registrados.</p>
          </div>
        )}
        ...
        {originData?.interaction_alerts_count ? originData.interaction_alerts_count > 0 && (
          <div className="p-4 bg-amber-50  border border-amber-200  rounded-2xl flex items-center gap-4">
             ...
             <div>
               <p className="text-sm font-black text-amber-900 ">Alerta de Seguridad Farmacológica</p>
