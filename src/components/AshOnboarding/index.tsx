...
const QUICK_REPLIES_BY_TYPE: Record<string, string[]> = {
  MEDICO: ['¿Tengo días de prueba gratis?', '¿Me ayudarás cuando entre al sistema?', '¿Qué pasa después de los 15 días?'],
  PACIENTE: ['¿Es gratis para siempre?', '¿Para qué sirve mi cédula?', '¿Cómo veré mis recetas?'],
  ENFERMERO: ['¿Tengo los 15 días gratis?', '¿Me darás el tour guiado?', '¿Qué es la matrícula?'],
  ADMIN: ['¿Cómo cuento los especialistas?', '¿Hay prueba para clínicas?', '¿Me ayudarás con la configuración?'],
};
...
function inferUserType(text: string): UserType {
  ...
  if (lower.includes('paciente'))
    return 'PACIENTE';
  if (lower.includes('enfermero') || lower.includes('enfermera') || lower.includes('enferer') || lower.includes('matrícula') || lower.includes('matricula'))
    return 'ENFERMERO';
...
export default function AshOnboarding() {
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      // Detectar tipo de usuario (priorizando el ya detectado)
      const detected = inferUserType(content);
      if (detected && !userType) setUserType(detected);
      const effectiveType = userType || detected;
