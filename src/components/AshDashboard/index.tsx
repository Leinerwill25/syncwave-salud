...
import { X, Send, Loader2, Bot, ChevronDown, Sparkles, HelpCircle, BookOpen } from 'lucide-react';
...
export default function AshDashboard() {
  ...
  // Detectar módulo actual (priorizando rutas más largas/específicas)
  const currentModuleData = Object.entries(MODULE_MAP)
    .sort((a, b) => b[0].length - a[0].length) // Las rutas más largas primero
    .find(([path]) => pathname === path || pathname?.startsWith(path + '/'))?.[1] || DEFAULT_MODULE;
  ...
  useEffect(() => {
    if (isOpen) {
      const moduleMsg: Message = {
        id: `module-${Date.now()}`,
        role: 'assistant',
        content: `Veo que te moviste a: **${currentModuleData.name}**. ¿Tienes alguna duda sobre esta pantalla?`,
      };
      
      // Solo lo añadimos si el último mensaje no es ya sobre este módulo
      setMessages(prev => {
        const lastMsg = prev[prev.length - 1];
        if (lastMsg?.content.includes(currentModuleData.name)) return prev;
        return [...prev, moduleMsg];
      });
    }
  }, [pathname, isOpen, currentModuleData.name]);
