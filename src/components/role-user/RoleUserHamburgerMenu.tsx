...
import { LayoutDashboard, CalendarDays, User, ClipboardList, FileText, MessageCircle, CheckSquare, Folder, ChevronRight, ChevronDown, Search, FileCheck, LogOut, Shield, Calendar, BarChart3, TrendingUp, CreditCard, Stethoscope, Menu, X } from 'lucide-react';
...
// Configuración de todos los módulos posibles (misma que RoleUserSidebar)
const ALL_MODULES: LinkItem[] = [
	...
	{
		href: '/dashboard/role-user/whatsapp',
		label: 'WhatsApp',
		icon: MessageCircle,
		showOnlyForRole: 'Asistente De Citas',
	},
...
export default function RoleUserHamburgerMenu() {
	...
	const loadSession = async () => {
		try {
			...
			console.error('[Role User Hamburger Menu] Error cargando sesión:', err);
			router.push('/login/role-user');
		} finally {
			setLoading(false);
		}
	...
	return (
		<>
			...
			<button
				...
				className="md:hidden fixed top-16 left-4 z-[9999] p-2 rounded-lg bg-white shadow-lg border border-teal-100 text-teal-600 hover:bg-teal-50 transition-colors"
				...
				<div className="flex flex-col gap-4 p-4 h-full">
					...
					<div className="relative">
						<input
							...
							className="w-full px-3 py-2 text-sm rounded-lg border border-blue-200 bg-blue-50 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-200 transition"
						/>
