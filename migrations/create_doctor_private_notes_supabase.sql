...
-- VERSIÓN OPTIMIZADA PARA SUPABASE
...
-- Índice compuesto para búsquedas rápidas por consulta y doctor
CREATE INDEX IF NOT EXISTS idx_doctor_private_notes_consultation_doctor ON doctor_private_notes(consultation_id, doctor_id);
...
DROP TRIGGER IF EXISTS trigger_update_doctor_private_notes_updated_at ON doctor_private_notes;
...
DROP POLICY IF EXISTS doctor_private_notes_select_policy ON doctor_private_notes;
DROP POLICY IF EXISTS doctor_private_notes_insert_policy ON doctor_private_notes;
DROP POLICY IF EXISTS doctor_private_notes_update_policy ON doctor_private_notes;
DROP POLICY IF EXISTS doctor_private_notes_delete_policy ON doctor_private_notes;

-- Política: Solo el doctor que creó la nota puede verla y modificarla
-- Usando auth.uid() para Supabase Auth
...
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'doctor_private_notes') THEN
        RAISE NOTICE 'Tabla doctor_private_notes creada exitosamente';
    ELSE
        RAISE EXCEPTION 'Error: La tabla doctor_private_notes no se pudo crear';
    END IF;
END $$;
