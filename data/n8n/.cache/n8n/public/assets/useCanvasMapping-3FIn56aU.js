...
import { D as TitledList_default } from "./ParameterInputList-z-vNHsn2.js";
...
import { r as useNodeDirtiness, t as useRunWorkflow } from "./useRunWorkflow-8yze1uFy.js";
...
import { a as useLogsStore, i as useExperimentalNdvStore, t as useCanvasOperations } from "./useCanvasOperations-BboXX3Gt.js";
...
var CanvasNodeStatusIcons_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		const groupedExecutionErrors = computed(() => {
			const errorCounts = executionErrors.value.reduce((acc, error$1) => {
				acc[error$1] = (acc[error$1] || 0) + 1;
				return acc;
			}, {});
			return Object.entries(errorCounts).map(([error$1, count$1]) => count$1 > 1 ? `${error$1} (x${count$1})` : error$1);
		});
		return (_ctx, _cache) => {
			return unref(isNotInstalledCommunityNode) && !isDemoRoute.value ? (openBlock(), createElementBlock("div", {
				...
				content: withCtx(() => [createVNode(TitledList_default, {
					...
					items: groupedExecutionErrors.value
				}, null, 8, ["title", "items"])]),
				default: withCtx(() => [createVNode(unref(N8nIcon_default), {
	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(unref(N8nPopover_default), {
				open: isPopoverVisible.value,
				"onUpdate:open": _cache[0] || (_cache[0] = ($event) => isPopoverVisible.value = $event),
				side: "top",
				width: "208px",
				...
				content: withCtx(() => [createBaseVNode("div", { class: normalizeClass(_ctx.$style.content) }, [(openBlock(true), createElementBlock(Fragment, null, renderList(colors.value, (color$2) => {
